    ...
    `}
      ...
      {result.diagram?.data && <section>
        ...
        <p className='mt-3 text-sm text-gray-500 italic'>
          🔷 If you need this diagram for future reference or revision. you can save it by taking a screenshots
        </p>
      </section>}
      ...
      {result.charts?.length > 0 &&
        <section>
          ...
          <p className='mt-3 text-xs text-gray-500 italic'>
            🔷 If you need this Charts for future reference or revision. you can save it by taking a screenshots
          </p>
        </section>
      }
...
function SectionHeader({ icon, title, color }) {
  ...
  return (
    <div className={`
      mb-4 px-4 py-2 rounded-lg bg-gradient-to-r ${colors[color]} font-semibold flex items-center gap-2
      `}>
      <span>{icon}</span>
      <span>{title}</span>
    </div>
